...
void usage()
{
	printf("ComputeUnitOptimizer-Cuda [OPTIONS...] [TARGETS]\n");
...
int main(int argc, char** argv)
{
	...
	for (unsigned int i = 0; i < args.size(); i++) {
		...
		try {
			if (optArg.equals("-h", "--help")) {
				...
				printf("ComputeUnitOptimizer-Cuda v" RELEASE "\n\n");
				printf("\nChecking... Secp256K1\n\n");
	...
	if (SetConsoleCtrlHandler(CtrlHandler, TRUE)) {
		ComputeUnitOptimizer* v;
		switch (searchMode) {
		case (int)SEARCH_MODE_MA:
		case (int)SEARCH_MODE_MX:
			v = new ComputeUnitOptimizer(inputFile, compMode, searchMode, coinType, gpuEnable, outputFile, useSSE,
				maxFound, rKey, rangeStart.GetBase16(), rangeEnd.GetBase16(), should_exit);
			break;
		case (int)SEARCH_MODE_SA:
		case (int)SEARCH_MODE_SX:
			v = new ComputeUnitOptimizer(hashORxpoint, compMode, searchMode, coinType, gpuEnable, outputFile, useSSE,
				maxFound, rKey, rangeStart.GetBase16(), rangeEnd.GetBase16(), should_exit);
			break;
		default:
